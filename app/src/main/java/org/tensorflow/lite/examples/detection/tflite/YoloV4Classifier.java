...
public class YoloV4Classifier implements Classifier {
    ...
    public float getObjThresh() {
        return RipenessActivity.MINIMUM_CONFIDENCE_TF_OD_API;
    }
    ...
    private static final int[] ANCHORS_TINY = new int[]{
            79,114,  95,119, 102,139,147,118,  95,185, 126,162};
...
//    }
        ...
        for (int i = 0; i < gridWidth;i++){
            ...
            if (score > getObjThresh()){
                ...
                final RectF rectF = new RectF(
                        ...
                        Math.min(bitmap.getWidth() - 1, xPos + w / 2),
                        Math.min(bitmap.getHeight() - 1, yPos + h / 2));
                Log.v("Get Classes", String.valueOf(detectedClass));
                Log.v("Get Classes", labels.get(detectedClass));
    ...
    private ArrayList<Recognition> getDetectionsForTiny(ByteBuffer byteBuffer, Bitmap bitmap) {
        ArrayList<Recognition> detections = new ArrayList<Recognition>();
        Map<Integer, Object> outputMap = new HashMap<>();
        outputMap.put(0, new float[1][13][13][45]);
        outputMap.put(1, new float[1][26][26][45]);
        Object[] inputArray = {byteBuffer};
        int detectedClass = 0;
        tfLite.runForMultipleInputsOutputs(inputArray, outputMap);
        ...
        float[][][][] new_box = new float[13][13][3][15];
        float[][][][] new_box2 = new float[26][26][3][15];
        int index1 = 0;
        int index2 = 0;
        int index3 = 0;
        ...
        {   index2 = 0;
            ...
            {   index3 = 0;
                for(int r = 0; r < 3; r++)
                {
                    for(int t = 0; t < 15; t++)
                    {
                        new_box[i][j][r][t] = bboxes[0][index1][index2][index3];
                        index3++;
                    }
                }
                index2++;
        ...
        {   index2 = 0;
            ...
            {   index3 = 0;
                for(int r = 0; r < 3; r++)
                {
                    for(int t = 0; t < 15; t++)
                    {
                        new_box2[i][j][r][t] = out_score[0][index1][index2][index3];
                        index3++;
                    }
                }
                index2++;
        ...
        for(int i = 0; i < 26; i++)
        {
            for(int j = 0; j < 26; j++)
            {
                for(int r = 0; r < 3; r++)
                {
                    for(int t = 0; t < 2; t++)
                    {
                        new_box2[i][j][r][t] = (float)(1.0/(1.0 + Math.exp(-(new_box2[i][j][r][t]))));
                    }
                }
            }
        }
            ...
            for(int j = 0; j < 13; j++)
            {
                for(int r = 0; r < 3; r++)
                {
                    for(int t = 0; t < 2; t++)
                    {
                        ...
                        new_box[i][j][r][t] = (float)(1.0/(1.0 + Math.exp(-(new_box[i][j][r][t]))));
                    }
                }
            }
        }
        ...
        for(int i = 0; i < 169; i++)
        {
            ...
            for(int j = 0; j < 3; j++)
            {
                ...
                if(objectness > 0.5)
                {
                    ...
                    for(int r = 5; r < 15; r++)
                    {
                        classes[r-5] = objectness * new_box[row][col][j][r];
                    }
                    for(int r = 0; r < 10; r++)
                    {
                        if(classes[r] < 0.5)
        ...
        for(int i = 0; i < 676; i++)
        {
            ...
            for(int j = 0; j < 3; j++)
            {
                objectness = new_box2[row][col][j][4];
                if(objectness > 0.5)
                {
                    float x = new_box2[row][col][j][0];
                    float y = new_box2[row][col][j][1];
                    float w = new_box2[row][col][j][2];
                    ...
                    for(int r = 5; r < 15; r++)
                    {
                        classes[r-5] = objectness * new_box2[row][col][j][r];
                    }
                    for(int r = 0; r < 10; r++)
                    {
                        if(classes[r] < 0.5)
...
//        }
        ...
        Log.v("Recognition1", String.valueOf(detections));
        final ArrayList<Recognition> recognitions = nms(detections);
        Log.v("Recognition2", String.valueOf(recognitions));
        return recognitions;
    }
